...
import { useSubmissionWebSocket } from '../../../contexts/SubmissionWebSocketContext';
...
const SUBMISSION_CONFIG: Record<SubmissionStatus, { color: string; icon: React.ReactNode }> = {
...
const SubmissionCard = ({ submission: sbms, onClick, isNew = false }: SubmissionCardProps) => {
  const [submission, setSubmission] = useState<Submission>(sbms);

  // ✅ One shared socket for the whole app — not one per card
  const { subscribe, unsubscribe } = useSubmissionWebSocket();

  const isTerminal = (status: SubmissionStatus) =>
    ![SubmissionStatus.RUNNING, SubmissionStatus.QUEUED, SubmissionStatus.PENDING].includes(status);
  ...
  useEffect(() => {
    // Only subscribe if the submission is still in-flight
    if (!sbms.id || isTerminal(sbms.status)) return;

    subscribe(sbms.id, (result) => {
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
        <Space size={16}>
          <Space size={5} align="center">
            <ClockCircleOutlined style={{ color: THEME.textSecondary, fontSize: 13 }} />
            <Text
              style={{
                color: THEME.textSecondary,
                fontSize: 13,
          ...
          <Space size={5} align="center">
            {!isTerminal(submission.status) === false && (
              <TestCountIcon passed={submission.passed} total={submission.total} />
            )}
            <Text
              style={{
                color: THEME.textSecondary,
...
const TestCountIcon = ({ passed, total }: { passed: number; total: number }) => {
  const allPassed = passed === total;
  return allPassed ? (
    <CheckCircleOutlined style={{ color: THEME.accentGreen, fontSize: 13 }} />
  ) : (
    <CloseCircleOutlined style={{ color: THEME.accentRed, fontSize: 13 }} />
  );
};
